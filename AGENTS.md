...
Use `.agents/priority-queue.jsonl` for append-only task priority snapshots. The latest row for each `task_id` is the current queue item. Priority rows help Hermes choose the highest-value next action across projects without replacing `.agents/task-log.jsonl`, handoff capsules, or job ledger attribution.
