...
# Compatibility constant only. ApprovalStore resolves the active profile at
# construction time so multiplexed/current-line HERMES_HOME scoping cannot be
...
class ApprovalStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.path = Path(path) if path is not None else get_hermes_home() / "approvals.db"
    ...
    def defer(
        ...
        *,
        ...
        """Release a lease without consuming the failure-attempt budget.

        Once a continuation has durably admitted its normal logical turn, a
        busy session or long-running model execution is healthy pending work,
        not a failed continuation attempt.
        """
        now = self._clock()
        with self._transaction():
            current = self._conn.execute(
            ...
            self._conn.execute(
                """UPDATE approval_continuations
                   SET state='retry', next_attempt_at=?, lease_owner=NULL,
                       lease_until=NULL, last_error=?, updated_at=?
                   WHERE id=?""",
                (
                    now + max(0.05, float(delay_seconds)),
                    str(error),
                    now,
                    continuation_id,
                ),
