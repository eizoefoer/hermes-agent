...
class ContinuationPending(RuntimeError):
    """The durable logical turn exists but has not reached terminal state."""
...
class ContinuationWorker:
    ...
    def run_once(self) -> Optional[Continuation]:
        ...
        except ContinuationPending as exc:
            return self.store.defer(
                item.id,
                self.worker_id,
                str(exc),
                delay_seconds=min(self.max_backoff, self.base_backoff),
            )
