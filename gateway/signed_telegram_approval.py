...
# Compatibility constant for callers that display the conventional location.
# Runtime construction below resolves the active profile dynamically instead
# of freezing HERMES_HOME at import time.
...
class ApprovalStore:
    def __init__(self, root: Path | None = None, now: Callable[[], float] = time.time):
        configured_root = os.environ.get("HERMES_APPROVAL_HOME")
        self.root = Path(
            root
            or configured_root
            or (get_hermes_home() / "approvals")
        )
