...
Selected base: `10e93c6ab958c7ec61cfc4416f4d4459e72ca8a7`

Live production reference: `f7c79efbac19ae18e8dee7c79a4e4c0935299b5f`

Phase 1 semantic reference: `accb7363d1565b28e1ce09ce329f1f3850326669`

Phase 1 reference base: `1bbb6e5bce56e721ab685af4cd87df21bbff4d35`

## Base selection

NousResearch `main` is 38 commits ahead of the live production revision, and
the live revision is its direct ancestor. Both trees declare version `0.20.6`.
The latest release tag (`v2026.8.27`) is older than the live revision, while
production itself follows untagged upstream `main`. The current upstream main
revision is therefore the selected replacement base; the divergent Phase 1
merge is used only as a semantic reference.

## v0.20.6 delta audit

The old Phase 1 reference changed 103 paths. Only 22 of those paths were also
changed between its base and the selected v0.20.6+ base. The important overlaps
...
The 0.20.5-to-0.20.6+ executor search found no new direct persistent reasoning
producer. Agent module splitting moved existing implementations without adding
a new ownership boundary. Background review, curator, manual compression and
prompt-size probes remain helper/evaluator work (H). Batch/eval execution
remains disposable (E). New and changed transports still normalize to
`MessageEvent` and enter `BasePlatformAdapter.handle_message`, so they inherit
the common gateway session-bound admission path (S). Expanded API run handling
is inside the API admission wrapper and retains API-scoped recovery.
