...
def test_profile_local_mcp_tool_is_visible_in_slash_worker(tmp_path):
    ...
    try:
        ...
        proc.stdin.flush()
        try:
            # MCP discovery imports and starts a real stdio child. Under the
            # canonical four-worker suite, startup can legitimately exceed the
            # old 10-second scheduling window without being stuck.
