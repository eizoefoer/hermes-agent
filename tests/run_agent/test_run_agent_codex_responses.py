...
def test_run_codex_stream_recovers_output_items_after_terminal_type_error(monkeypatch):
    agent = _build_agent(monkeypatch)
    calls = {"create": 0}
    item = _codex_message_response("item survived").output[0]

    def _fake_create(**kwargs):
        calls["create"] += 1
        return _codex_message_response("fallback should not run")
    ...
    assert response.output[0].content[0].text == "item survived"


def test_run_codex_stream_synthesizes_text_after_terminal_type_error(monkeypatch):
    agent = _build_agent(monkeypatch)
    calls = {"create": 0}
